 ...
 * Created Date: Friday, April 21st 2023, 3:20:34 pm
 * Author: Olimpiev Y. Y.
 * -----
...
void GatherResultMatrix(gsl_matrix* subResult, const int gridParam[2], MPI_Comm comm, gsl_matrix* result) {
    int coords[2];
    int rank = 0;
    ...
    if (rank == 0) {
        ...
        gsl_matrix_view resMatrixView = gsl_matrix_submatrix(result, 0, 0, subResult->size1, subResult->size2);
        ...
        for (int p = 1; p < size; p++) {
            ...
            resMatrixView = gsl_matrix_submatrix(result, 
                result->size1 / gridParam[0] * coords[1], 
                result->size2 / gridParam[1] * coords[0], 
                subResult->size1,
                subResult->size2);
            gsl_matrix_memcpy(&resMatrixView.matrix, subResult);
        }
...
gsl_matrix* DistributeMatrixesOverTheGrid(gsl_matrix* matrixes[2], MPI_Comm comm, const int gridParam[2]) {
    ...
    gsl_matrix* result = (rank == 0) ? gsl_matrix_calloc(matrixes[0]->size1, matrixes[1]->size2): NULL;
    GatherResultMatrix(subResult, gridParam, comm, result);
    return result;
}
...
gsl_matrix* GridTopologyMatrixMultiplication(gsl_matrix* A, gsl_matrix* B, size_t gridWidth, size_t gridHeight) {
    ...
    // Build topolody and communicator.
    MPI_Comm gridComm;
    int dimSizes[2] = {gridWidth, gridHeight};

    int wrapAround[2] = {0, 0};
    if (MPI_SUCCESS != MPI_Cart_create(MPI_COMM_WORLD, 2, dimSizes, wrapAround, 0, &gridComm)) {
    ...
    return DistributeMatrixesOverTheGrid(matrixes, gridComm, dimSizes);
}
...
int main(int argc, char *argv[]) {
    ...
    if (0 == rank) {
        gsl_matrix* answer = gsl_matrix_calloc(matrixA->size1, matrixB->size2);
        assert(answer);

        gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, matrixA, matrixB, 0.0, answer);
        fprintf(stdout, "Expected result:\n");
        pretty_gsl_matrix_fprintf(stdout, answer, "%lf ");

        gsl_matrix_free(answer);
    }
