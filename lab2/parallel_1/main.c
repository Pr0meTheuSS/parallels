...
void openmp_gsl_blas_ddot(gsl_vector* x, gsl_vector* y, double* res, size_t* jobSizes, size_t* displs) {
    ...
    assert(res);
    double resPart = 0.0;
    #pragma omp parallel shared(resPart)
    {
        int rank = omp_get_thread_num();
...
void openmp_gsl_blas_daxpy(double alpha, gsl_vector* x, gsl_vector* y, size_t* jobSizes, size_t* displs) {
    ...
    #pragma omp parallel shared(alpha ,x ,y)
    {
        int rank = omp_get_thread_num();
...
double ConjugateGradientsMethodIteration(
    ...
    openmp_gsl_blas_ddot(tmpVec, z, &alpha, jobSizes, displs);
    // Calc (r_n, r_n) / (A * z_n, z_n).
    alpha = tmp / alpha;
    // Calc x_(n + 1) = x_n + aplha * z_n.
    openmp_gsl_blas_daxpy(alpha, z, x, jobSizes, displs);
    // Calc r_(n + 1) = r_n - aplha * (A * z_n).
    openmp_gsl_blas_daxpy(-alpha, tmpVec, r, jobSizes, displs);
    // Calc (r_(n + 1), r_(n + 1)).
    openmp_gsl_blas_ddot(r, r, &betta, jobSizes, displs);
    // Calc betta_(n + 1) = (r_(n + 1), r_(n + 1)) / (r_n, r_n).
    betta /= tmp;

    gsl_vector_set_zero(tmpVec);
    openmp_gsl_blas_daxpy(betta, z, tmpVec, jobSizes, displs);
    openmp_gsl_blas_daxpy(1.0, r, tmpVec, jobSizes, displs);
